...
# Define server logic ----
  ...
  output$correct_rate_plot <- renderPlot({
    ...
    ggplot(tmp, mapping = aes_string(x="correct_rate", y="ratio", fill=as.character(input$feature))) + geom_bar(stat = "identity",position = "dodge", width = 1.5)
  })
}
